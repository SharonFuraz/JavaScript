...
recetas.forEach((comida)=>console.log("Nuestras recetas son: "+comida.receta))
...
const buscarRecetas=recetas.filter((comida)=>comida.preparacion=="harina");
if(consulta=="harina"){
  console.log(buscarRecetas)
  console.log(faina)
  console.log(noquis)
}
let cards=document.getElementsByClassName("index");
for (let i = 0; i < recetas.length; i++) {
    let card=document.createElement ("div");
    card.className="card col-md-3";
    card.innerHTML=`
    <div class="card-body">
    <h5 class="card-title">${recetas[i].receta}</h5>
    <p class="card-text">${recetas[i].primerIngrediente},${recetas[i].cantidadPrimerIngrediente},${recetas[i].unidadDeMedidaPrimer},${recetas[i].segundoIngrediente}, ${recetas[i].cantidadSegundoIngrediente},
    ${recetas[i].unidadDeMedidaSegundo},${recetas[i].tercerIngrediente}, ${recetas[i].cantidadTercerIngrediente},${recetas[i].unidadDeMedidaTercer},${recetas[i].cuartoIngrediente}, 
    ${recetas[i].cantidadCuartoIngrediente},${recetas[i].unidadDeMedidaCuarto} ,${recetas[i].quintoIngrediente}, ${recetas[i].cantidadQuintoIngrediente},${recetas[i].unidadDeMedidaQuinto},
    ...
    <p class="card-text">${recetas[i].preparacion}</p>
    <p class="card-text">Costo: $ ${recetas[i].costo}</p>
